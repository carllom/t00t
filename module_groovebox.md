...
but its handler is a stub comment ("Kit / preset switching — to be added
with the 909 kit").
...
Mod Source" (the citation here previously said `engine.md`, which has no
sequencer content; §1 above already cites `architecture.md` correctly).

**As-built, this section is partly stale** (flagged in the doc's intro, but
worth restating here): a basic MIDI-clock-driven step sequencer already
exists (`patterns.h` + the `seq_*` state/functions in
...
Most of these are now settled by what shipped; left in place as the original
questions, with the as-built answer noted per item (no "Settled Decisions"
section exists in this doc to move them to).
...
3. **Metal bank** — cowbell → hats → cymbal; measure CPU on the 6-osc path.
   ...
   §7's numbers are still estimates, per the doc intro.)
...
5. **909 kit** — sample-based hats/cymbals (`VT_DRUM_SAMPLE`) + BD click; retune
   ...
   (**not built** — still P3, matches the doc's own intro/§3.3.)
