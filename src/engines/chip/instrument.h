...
// The frame-rate instrument table VM (sid.md §6): "ADSR + vibrato + three
...
//
// Format notes not fully pinned down by sid.md itself, decided here:
//
// - WaveRow.note is a *relative* semitone offset from the played note
//   (arpeggios), not an absolute note -- sid.md says "note abs/rel" without
...
struct Instrument {
    ...
    bool     uses_filter;        // request a bus at trigger time (sid.md §5.2 bind_filter)
