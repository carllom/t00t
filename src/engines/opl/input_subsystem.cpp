...
// OPL's Input subsystem: the module-specific tail of the Input pipeline --
...
// mapping table and Handlers are fully this module's own. Own controller,
// not the shared src/midi/midi_controller.cpp: this engine's VoiceParams
// carries a patch pointer, not a VoicePreset the shared controller's
// presets.h shape expects.
//
// Patch select: Program Change picks `OPL_PATCHES[value.index % OPL_PATCH_COUNT]`
// (patches.h) -- always at least one entry, hand-authored and checked in, so
// patch select is unconditional here, not gated behind a "has this been
// generated" macro.
...
static const OplPatch *channel_patch[NUM_CHANNELS];  // defaults to &OPL_PATCH_LEAD, Program Change overrides it
static uint8_t channel_program[NUM_CHANNELS];        // OPL_PATCHES[] index, for ui_state.program
static int16_t channel_mod_wheel[NUM_CHANNELS];      // CC1, Q15 -- scales vibrato depth (live)
...
// Push mod wheel (CC1) to every held voice on a channel -- live vibrato
// depth scaling.
...
static void set_note(VoiceParamBlock &shadow, const InputValue &value) {
    if (value.note_on) {
        ...
        vp.note = value.note;  // key/rate scaling need the raw note, not just phase_inc
...
// picks a patch by index (wrapped into OPL_PATCH_COUNT, always >= 1) for
...
static void set_patch(VoiceParamBlock &, const InputValue &value) {
    uint8_t idx = (uint8_t)(value.index % OPL_PATCH_COUNT);
    channel_patch[value.channel] = OPL_PATCHES[idx];
...
static_assert(input_table_declares_capabilities(kMappingTable, kCapabilities),
              "opl mapping table entry uses an InputCategory not in kCapabilities");
...
void midi_controller_init() {
    ...
    for (uint8_t ch = 0; ch < NUM_CHANNELS; ch++) {
        ...
        channel_mod_wheel[ch] = 0;  // resting position -- vibrato still plays at its patch-configured depth
    }
