...
// FM status display (Core 0, low priority): the shared Widget/Header/Page
// library (#124) applied to FM, same Performance-page layout as OPL's own
// (src/engines/opl/display.cpp) -- Header's Resource bar folding voices+CPU,
// the current preset, and FXMIX/FX P1/FX P2/FX type/MOD as compact Value
// bars/Label -- plus a DIAG page carrying the exact-value detail Resource
// bar deliberately sacrifices (CPU%, per-voice activity, last note, the
// six-operator carrier/modulator/feedback indicator, and the per-voice
// multitimbral grid) -- reachable via the rotary encoder (encoder_nav.h)
// where one's wired (HAS_ENCODER), Performance-only otherwise.
...
// Performance page rows -- identical shape to OPL's own.
...
static constexpr int GRID_ROW0 = 166;
static constexpr int DIAG_BAR_W = 232;
static constexpr int ALGO_CELL_PITCH = 24, ALGO_CELL_W = 20, ALGO_CELL_H = 16;

// Per-voice multitimbral grid: voices 0..GRID_VOICES-1 (MAX_VOICES=16 is too
// many for a legible one-cell-per-voice grid alongside everything else on
// this panel -- voice_alloc's allocate() always scans from v=0 first
// (src/voice_alloc.cpp), so this covers whatever's sounding for anything up
// to 8-note polyphony). 4 columns x 2 rows, "voice:channel/patch" (channel
// 1-based), e.g. "3:2/07" -- voice 3, MIDI channel 2, patch index 7.
...
struct GridCellKey {
    ...
    bool operator==(const GridCellKey &o) const {
        return active == o.active && channel == o.channel && program == o.program;
    }
...
static const FmPatch *last_algo_patch = nullptr;
static ValueRow<GridCellKey> grid_cells[GRID_VOICES];

static void draw_algo_indicator(const FmPatch *patch) {
...
void display_task() {
    ...
    header_draw_module_name(hdr, "FM", LCD_W, COL_TITLE_FG, COL_TITLE);
    ...
    if (page == PAGE_PERFORMANCE) {
        ...
        for (uint32_t v = 0; v < GRID_VOICES; v++) {
            bool active_v = snd & (1u << v);
            FmVoiceUiState vs;
            fm_voice_ui_state(v, &vs);
            GridCellKey key{ active_v, vs.channel, vs.program };
            if (!grid_cells[v].initialized || grid_cells[v].value != key) {
                int col = (int)(v % 4), row = (int)(v / 4);
                int x = col * GRID_W, y = GRID_ROW0 + row * GRID_ROW_H;
                char text[GRID_CH + 1];
                if (active_v) {
                    snprintf(text, sizeof(text), "%lu:%u/%u", (unsigned long)v, vs.channel + 1,
                             vs.program);
                } else {
                    text[0] = '\0';
                }
                value_row_draw_value(grid_cells[v], key, text, x, y, GRID_CH, 1,
                                      active_v ? COL_VALUE : COL_LABEL, COL_BG);
            }
        }
    }
