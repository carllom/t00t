...
// sustains the vowel; the vowel new notes on a channel use is selectable
// two ways -- Program Change, or CC20 (banded into VOWEL_COUNT zones, same
...
// BSP encoder range so the same physical knob can drive vowel selection
...
static uint8_t channel_vowel[NUM_CHANNELS];   // Vowel (phonemes.h), set by program change
...
static void midi_voice_on(VoiceParamBlock &shadow, int v, uint8_t note, uint8_t velocity, uint8_t channel) {
    ...
    vp.phoneme = channel_vowel[channel];
    ...
    ui_state.program = channel_vowel[channel];
}
    ...
    for (uint32_t i = 0; i < len; i++) {
        ...
        switch (ev.type) {
            ...
            case MIDI_CC: {
                switch (ev.data1) {
                    case 10:  // pan (CC10) — 0=full left, 64=center, 127=full right
                        ...
                        changed = true;
                        break;
                    case 20:  // vowel select — split range into VOWEL_COUNT bands (see
                              // header comment: alternative to Program Change for
                              // controllers that can't send it from a live encoder)
                        channel_vowel[ev.channel] = (uint8_t)((uint32_t)ev.data2 * VOWEL_COUNT / 128u);
                        ui_state.program = channel_vowel[ev.channel];
                        ui_state.last_channel = ev.channel;
                        break;
                    ...
                    case 75:  // effect param 2: delay time / reverb damping
                        ...
                        changed = true;
                        break;
                    default:  break;  // other CCs — formant_shift/bandwidth/rate/jitter/shimmer land in P2+
                }
                break;
            }
            case MIDI_PROGRAM_CHANGE: {
                ...
                // channel_program.
                channel_vowel[ev.channel] = ev.data1 % VOWEL_COUNT;
                ui_state.program = channel_vowel[ev.channel];
                ui_state.last_channel = ev.channel;
                break;
            }
