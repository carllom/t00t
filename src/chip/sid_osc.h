...
//
// TOPOLOGY-FREE (sid.md §4, hard rule from the first commit): this knows
...
// this file departs from sid.md, the reference is why -- each such place is
// commented, because the doc is otherwise the thing a reader would trust.

// ---------------------------------------------------------------------------
// Accumulator format
//
// sid.md §4.1: the SID's 24-bit accumulator held as Q24.8 in a uint32_t, i.e.
...
//
//   sid.md §4.1 gives "inc = freq_reg * 5805  (44.1 kHz, PAL)". 5805 is
...
// one constant. sid.md does not mention it; §3's test puts it firmly on the
...
//
// sid.md §4.2 says "23-bit LFSR, taps 22/20/16/13/11/7/4/2". Those eight
...
//
// sid.md §4.2: "Noise needs a clock count per sample, not a single clock: at
...
// Hard sync
//
// sid.md §4.1: "Hard sync is plain uint32_t wrap detection -- carry out of bit
...
// Combined waveforms.
//
// sid.md §13.5 settles this as "AND first (P1), LUTs later (P6)", on the
...
struct SidOsc {
    ...
    // own sub-oscillator (sid.md §4.4). Fusing them would force one of the two
    ...
    // which is where sid.md §4.4's "5-8 c/f versus 45-65 for a whole voice"
    // comes from.
