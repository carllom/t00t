...
//
// The SID filter is a two-integrator-loop SVF (module_chip.md §2). reSID's
// own 8580 path is
...
// this uses a cutoff LUT plus saturation instead; the LUT is measured off
// reSID by tools/sid_ref/resid_probe.cpp and fitted by
// tools/fit_6581_filter.py. See sid_tables.h for the provenance.

// ---------------------------------------------------------------------------
// Mode mask.
//
// FilterMode here is a 3-bit *mask* (module_chip.md §5.1), because the SID
...
// coefficient, via the model's own table (module_chip.md §5.1): a pluggable
// LUT rather than svf_compute_f_half's linear map, so 6581 vs 8580 is a
// table swap, not a code fork.
...
// Saturation.
//
// The 6581's filter nonlinearity is signal-path, so it is kept
// (module_chip.md §5.1, §3). It is also what makes §5.2's shared-bus
// intermodulation a real tonal property rather than a claim -- voices
// summed before a *linear* filter do not intermodulate at all.
//
...
inline int32_t sid_filter_saturate(int32_t x) {
    ...
    // multiply rather than two runtime int64_t/int32_t divides -- the
    // target has no hardware 64-bit divide, so this is a hot-path
    // optimization. The approximation is close enough to the divide-based
    // formula to stay within this curve's existing tolerance: it's a cheap
    // qualitative approximation already, not a reSID-fitted one.
...
//
// The passive RC network between the SID and the AV connector is a one-pole
// low-pass (module_chip.md §10). It sits before the speaker simulation stage
// and after the chip. reSID models it as ExternalFilter, a high-pass at
// ~16 Hz and a low-pass at ~16 kHz; the high-pass matters for DC removal --
// the 6581's 0x380 wave zero puts a real DC step on every gate, and without
// this it lands in the mix.
// ---------------------------------------------------------------------------
