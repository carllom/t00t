// Host-buildable visual preview of FM's Performance/DIAG Page layout
// (src/engines/fm/display.cpp) against representative synthetic telemetry
...
// state that isn't host-buildable), same reason preview_opl_display.cpp
// (and test_header.cpp and friends) call Widgets directly rather than
// exercising a real engine's display.cpp.
...
static const uint16_t COL_TITLE     = gfx_rgb(30, 150, 70);
...
static constexpr int GRID_ROW0 = 166, GRID_ROW_H = 14, GRID_W = 240 / 4, GRID_CH = 9;
static constexpr int ALGO_CELL_PITCH = 24, ALGO_CELL_W = 20, ALGO_CELL_H = 16;
static constexpr int MAX_VOICES = 16;
static constexpr int FM_NUM_OPS = 6;
...
// op i is a carrier for i in {2,4,5} in this synthetic algorithm, feedback
// on op 0 only -- just a representative six-cell pattern, not a real patch.
static void draw_algo_indicator() {
    for (int i = 0; i < FM_NUM_OPS; i++) {
        bool carrier = (i == 2 || i == 4 || i == 5);
        uint16_t fg = carrier ? COL_CARRIER : COL_MODULATOR;
        uint16_t fill = (i == 0) ? COL_FEEDBACK : fg;
...
int main() {
    ...
    char text[24];
    snprintf(text, sizeof(text), "#%-3d %.10s", 12, "BRASS 1");
    value_row_draw_value(preset_row, 12, text, 4, PRESET_Y, 28, 1, COL_VALUE, COL_BG);
    ...
    write_ppm("/tmp/claude-1000/-home-carl-t00t/9b60f263-674b-4d9d-aa37-7211fbf4344a/scratchpad/fm_performance.ppm");
    ...
    bool active[MAX_VOICES] = { true, true, false, true, false, true, false, false,
    ...
    for (int v = 0; v < GRID_VOICES; v++) {
        ...
        if (cell_active) {
            snprintf(gtext, sizeof(gtext), "%d:%d/%02d", v, v % 3 + 1, v + 1);
        } else {
            gtext[0] = '\0';
        }
        value_row_draw_value(grid_cells[v], v, gtext, x, y, GRID_CH, 1,
                              cell_active ? COL_VALUE : COL_LABEL, COL_BG);
    }

    write_ppm("/tmp/claude-1000/-home-carl-t00t/9b60f263-674b-4d9d-aa37-7211fbf4344a/scratchpad/fm_diag.ppm");
