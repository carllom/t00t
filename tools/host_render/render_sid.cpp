// render_sid -- the t00t side of the F0 comparison (sid.md §11.1, CHIP_STRICT).
//
...
// as a second topology over the same validated primitives (sid.md §4, §11.1).
//
...
#define CHIP_STRICT 1   // sid.md §11.1: velocity scaling must compile out here
...
// The strict 3-voice / 1-filter topology (sid.md §11.1's "$D400-$D418").
struct StrictSid {
    SidVoice   voice[3];
    SidFilter  filter;
    ...
    void write_reg(uint8_t reg, uint8_t val) {
        if (reg <= 0x14) {
            ...
            // for MIDI retriggering (sid.md §4.3's deliberate divergence).
    ...
    int32_t tick() {
        ...
        // 4-bit master volume DAC, linear (sid.md does not model this DAC's
