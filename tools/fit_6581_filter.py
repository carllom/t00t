...
sid.md §5.1 asks for "a pluggable cutoff LUT instead of svf_compute_f_half's
...
"measured from reSID" provenance:
  ...
  * the 8-bit envelope and 12-bit waveform R-2R DAC tables, derived from the
    ...
    r2r_vbit), which sid.md does not mention but §3's signal-path test keeps;
  * the C64 board output network coefficients (sid.md §10's "free bonus");
...
def svf_response(f_half_q15, q_q15, mode_mask, freqs):
    ...
    # combination sid.md §5.1 exists to support.
    out = np.zeros_like(z)
    if mode_mask & 0x1:
        out = out + z * s0
    if mode_mask & 0x2:
...
def emit(args, sha, res_q, res_f, f_full, fc_ref):
    # --- 8580 ---------------------------------------------------------------
    # sid.md §13.6: "6581 first, LUT-based, so 8580 is a table swap plus a
    # saturation bypass." The 8580 table is NOT measured here -- P6 owns that
    # (sid.md §1). What is emitted is reSID's own closed-form 8580 mapping,
...
//
// PROVENANCE. sid.md §5.1: "The 6581 cutoff curve varied enormously between
...
'''
    ...
    body.append(fmt_table(
        ...
        "// 8580 cutoff. PLACEHOLDER, not measured -- sid.md §1 puts the 8580 model\n"
    ...
    body.append(fmt_table(
        ...
        "// sid.md does not mention the DACs; §3's test (does it operate inside the\n"
...
// C64 board output network (sid.md §10's "free bonus"): the passive RC between
...
// sid.md §5.2 depends on for shared-bus intermodulation to be real.
