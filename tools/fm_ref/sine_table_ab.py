...
module_fm.md §3.5 justified dropping interpolation with an estimated ~-72 dBc
...
the spur floor rises with modulation index. Since fixing history_fm.md §1.1(a) is
...
def render(f_c, f_m, beta, mode):
    ...
    offset: 2**32 phase units == 2*pi radians, matching the contract history_fm.md §2
