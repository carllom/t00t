...
//          sid.md §4.2's "AND first, LUTs at P6" deferral gets a *number*.
//   dac    reSID's own 8-bit (envelope) and 12-bit (waveform) R-2R DAC tables.
//          sid.md does not mention these at all; they are signal-path and
...
//
// This is the domain where sid.md §4.2 is wrong and says so loudly: it lists
// "taps 22/20/16/13/11/7/4/2", but reSID's clock_shift_register() is
// bit0 = bit22 ^ bit17 -- two taps. The eight positions in sid.md's list are
...
// a quiet 6581 note sounds dirty rather than merely quiet. sid.md's §3
